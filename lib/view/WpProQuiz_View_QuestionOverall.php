...
<div class="wrap wpProQuiz_questionOverall">
	<h2>
		<?php printf(__('Quiz: %s', 'Bestwebsite-Pro-Quiz'), $this->quiz->getName()); ?>

		<?php if(current_user_can('wpProQuiz_edit_quiz')) { ?>
			<a class="add-new-h2" href="?page=wpProQuiz&module=question&action=addEdit&quiz_id=<?php echo $this->quiz->getId(); ?>"><?php _e('Add question', 'Bestwebsite-Pro-Quiz'); ?></a>
		<?php } ?>
        <?php if (current_user_can('wpProQuiz_import')) { ?>
            <a class="add-new-h2 wpProQuiz_import" href="#"><?php echo __('Import', 'Bestwebsite-Pro-Quiz'); ?></a>
        <?php } ?>
	...
	<p>
		<a class="" href="admin.php?page=wpProQuiz"><?php _e('back to overview', 'Bestwebsite-Pro-Quiz'); ?></a>
	</p>

	<p>
		<?php if(current_user_can('wpProQuiz_edit_quiz')) { ?>
			<a class="button-secondary" href="admin.php?page=wpProQuiz&action=addEdit&quizId=<?php echo $this->quiz->getId(); ?>"><?php _e('Edit quiz', 'Bestwebsite-Pro-Quiz'); ?></a>
			<a class="button-secondary" id="sortQuestionBtn" href="#"><?php _e('Sort Question', 'Bestwebsite-Pro-Quiz'); ?></a>
			<a class="button-secondary" href="#" id="wpProQuiz_questionCopyBtn"><?php _e('Copy questions from another Quiz', 'Bestwebsite-Pro-Quiz'); ?></a>
		<?php } ?>
		...
		<div id="wpProQuiz_questionCopy_box" style="display: none;">
			<div class="wpProQuiz_questionCopy">
				<form action="admin.php?page=wpProQuiz&module=question&quiz_id=<?php echo $this->quiz->getId(); ?>&action=copy_question" method="POST">
					<h3 style="margin-top: 0;"><?php _e('Copy questions from another Quiz', 'Bestwebsite-Pro-Quiz'); ?></h3>
					<p><?php echo __('Here you can copy questions from another quiz into this quiz. (Multiple selection enabled)', 'Bestwebsite-Pro-Quiz'); ?></p>
					...
					<div style="padding: 20px; display: none;" id="loadDataImg">
						...
						<?php echo __('Loading', 'Bestwebsite-Pro-Quiz'); ?>
					</div>
					...
					<input class="button-primary" name="questionCopy" value="<?php echo __('Copy questions', 'Bestwebsite-Pro-Quiz'); ?>" type="submit">
				</form>
			</div>
		</div>
		...
		<div id="wpProQuiz_setQuestionCategoryList_box" style="display: none;">
			<div class="wpProQuiz_setQuestionCategoryList">
				<form action="#" method="POST">
					<h3 style="margin-top: 0;"><?php _e('Set Question Categories', 'Bestwebsite-Pro-Quiz'); ?></h3>
					<p><?php _e('Sets multiple question categories', 'Bestwebsite-Pro-Quiz'); ?></p>
					...
					<div style="margin-bottom: 10px;">
						<p class="description">
							<?php _e('You can assign classify category for a question.', 'Bestwebsite-Pro-Quiz'); ?>
						</p>
						<p class="description">
						...
						<div>
							<select name="category">
								<option value="-1">--- <?php _e('Create new category', 'Bestwebsite-Pro-Quiz'); ?> ----</option>
								<option value="0" selected="selected">--- <?php _e('No category', 'Bestwebsite-Pro-Quiz'); ?> ---</option>
								<?php
						...
						<div style="display: none;" id="categoryAddBox">
							...
							<input type="button" class="button-secondary" name="" id="categoryAddBtn" value="<?php _e('Create', 'Bestwebsite-Pro-Quiz'); ?>">
						</div>
						<div id="categoryMsgBox" style="display:none; padding: 5px; border: 1px solid rgb(160, 160, 160); background-color: rgb(255, 255, 168); font-weight: bold; margin: 5px; ">
							<?php _e('Category saved', 'Bestwebsite-Pro-Quiz'); ?>
						</div>
					</div>

					<input class="button-primary" name="setCategoriesStart" id="setCategoriesStart" value="<?php _e('Save', 'Bestwebsite-Pro-Quiz'); ?>" type="button">
	...
	protected function showSortQuestionBox() {
		...
		<div id="wpProQuiz_sortQuestion_box" style="display: none;">
			<div id="wpProQuiz_sortQuestion">
				<h4><?php _e('Sort questions', 'Bestwebsite-Pro-Quiz'); ?></h4>
				<p>
            ...
            <div class="wpProQuiz_exportList">
                <form action="<?php echo admin_url('admin.php?page=wpProQuiz&module=questionExport&action=export&noheader=true'); ?>" method="POST">
                    ...
                    <input class="button-primary" name="exportStart" id="exportStart"
                           value="<?php echo __('Start export', 'Bestwebsite-Pro-Quiz'); ?>" type="submit">
                </form>
            </div>
        </div>
        ...
        <div id="wpProQuiz_questionImportList_box" style="display: none;">
            <div class="wpProQuiz_importList">
                <form action="<?php echo admin_url('admin.php?page=wpProQuiz&module=questionImport&action=preview&quizId='.$this->quiz->getId()); ?>" method="POST"
                    ...
                    <h3 style="margin-top: 0;"><?php _e('Import', 'Bestwebsite-Pro-Quiz'); ?></h3>

                    <p><?php _e('Import only files from known and trusted sources.', 'Bestwebsite-Pro-Quiz'); ?></p>
                    <p><?php echo sprintf(__('Supported formats: %s', 'Bestwebsite-Pro-Quiz'), implode(', ', $this->importFormats['extensions'])) ?></p>
                    ...
                    <div style="margin-bottom: 10px">
                        ...
                        <input type="file" name="import" accept="<?php echo implode(',', $this->importFormats['accept']) ?>"
                               required="required"> <?php printf(__('Maximal %d MiB', 'Bestwebsite-Pro-Quiz'), $uploadMB); ?>
                    </div>
                    <input class="button-primary" name="importStart" id="importStart"
                           value="<?php _e('Start import', 'Bestwebsite-Pro-Quiz'); ?>" type="submit">
                </form>
            </div>
        </div>
