...
class WpProQuiz_Controller_Toplist extends WpProQuiz_Controller_Controller
{
    ...
    private function handleAddInToplist(WpProQuiz_Model_Quiz $quiz)
    {
        if (!wp_verify_nonce($this->_post['token'], 'wpProQuiz_toplist')) {
            return array('text' => __('An error has occurred.', 'Bestwebsite-Pro-Quiz'), 'clear' => true);
        }
        ...
        if ($userId > 0) {
            if ($toplistMapper->countUser($quizId, $userId, $clearTime)) {
                return array('text' => __('You can not enter again.', 'Bestwebsite-Pro-Quiz'), 'clear' => true);
            }
            ...
            if (empty($name) || empty($email) || filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
                return array('text' => __('No name or e-mail entered.', 'Bestwebsite-Pro-Quiz'), 'clear' => false);
            }

            if (strlen($name) > 15) {
                return array('text' => __('Your name can not exceed 15 characters.', 'Bestwebsite-Pro-Quiz'), 'clear' => false);
            }
            ...
            if ($quiz->isToplistDataCaptcha()) {
                ...
                if ($captcha->isSupported()) {
                    if (!$captcha->check($prefix, $captchaAnswer)) {
                        return array('text' => __('You entered wrong captcha code.', 'Bestwebsite-Pro-Quiz'), 'clear' => false);
                    }
                }
            }
        }
    ...
    public static function ajaxAddInToplist($data)
    {
        ...
        if ($r === true) {
            $r = array('text' => __('You signed up successfully.', 'Bestwebsite-Pro-Quiz'), 'clear' => true);
        }
