...
class WpProQuiz_Controller_Question extends WpProQuiz_Controller_Controller
{
    ...
    private function showActionHook()
    {
        ...
        add_screen_option('per_page', array(
            'label' => __('Questions', 'Bestwebsite-Pro-Quiz'),
            'default' => 20,
    ...
    private function addEditQuestion($quizId)
    {
        ...
        if ($questionId && $questionMapper->existsAndWritable($questionId) == 0) {
            WpProQuiz_View_View::admin_notices(__('Question not found', 'Bestwebsite-Pro-Quiz'), 'error');
        ...
        if (isset($this->_post['template']) || (isset($this->_post['templateLoad']) && isset($this->_post['templateLoadId']))) {
            ...
            if (isset($this->_post['submit'])) {
                if ($questionId) {
                    WpProQuiz_View_View::admin_notices(__('Question edited', 'Bestwebsite-Pro-Quiz'), 'info');
                } else {
                    WpProQuiz_View_View::admin_notices(__('Question added', 'Bestwebsite-Pro-Quiz'), 'info');
                }
        ...
        $view->header = $questionId ? __('Edit question', 'Bestwebsite-Pro-Quiz') : __('New question', 'Bestwebsite-Pro-Quiz');
    ...
    private function getPostQuestionModel($quizId, $questionId)
    {
        ...
        if (empty($post['title'])) {
            ...
            $post['title'] = sprintf(__('Question: %d', 'Bestwebsite-Pro-Quiz'), $count + 1);
        }
    ...
    public function copyQuestion($quizId)
    {
        ...
        WpProQuiz_View_View::admin_notices(__('questions copied', 'Bestwebsite-Pro-Quiz'), 'info');
